...
- *Bubble sort:* Time complexity: O(n^2), Space complexity: O(1). (It compares two neighbouring elements and if the second is lower, then it swaps their positions. Each value is compared to the rest of the values once => O(n^2) time complexity.)
...
- *Insertion sort:*  used when the list of items is nearly sorted, and in small data-set. Time complexity: O(n^2) (in almost sorted: O(n)). (It takes the first two elements and sorts them (swaps if necessary), then it takes the next element and puts it to the correct place within the already ordered part.)
- *Merge sort:* one of the most efficient sorting algorithms. It uses "Divide&Conquer" approach. (It splits the list in the middle until the individual numbers/items, then it takes first two and combines them according to their value, then the second two... Then it compares the two merged...) It doesn´t compare each item to each item -> Time complexity: O(n log n), space complexity: O(n) (more space is needed).
- *Stable vs. unstable algorithms:* A sorting algorithm is said to be stable if two objects with equal keys appear in the same order in sorted output as they appear in the input array to be sorted. (*Stable sorting algorithms:* Merge sort, Insertion sort, Buble sort,... *Unstable sorting algorithms:* Heap sort, Selection sort, Quick sort, Tree sort,...). Stable algorithms have higher big-O CPU and/or memory usage than ustable algorithms. (A good compromise stable algorithms is a binary tree sort).
